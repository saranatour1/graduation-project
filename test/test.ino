...
#define Offset -0.89         //deviation compensate // check ASAP 
...
int time_to_turn_off_Grow_lights=1;

const long Interval=5UL*1000UL;
const long Interval_pH=5UL*1000UL;
const long Interval_tds=5UL*1000UL;
...
void loop(){
 ...
 // Water_level = digitalRead(Level_sensor_pin);
  ...
  if (currentMillis_pump - previousMillis_pump >= 5000) // to repeat every hour for the interval of 1 minitue
  {
    ...
    if (pumpState == HIGH && (sec==43||sec==44||sec==45||sec==46||sec==47||sec==48||sec==49 ||sec== 50) && Water_level == 1 )
    { pumpState = LOW; digitalWrite(Pump_pin, pumpState); Serial.println(" time to pump water for 1 minitue");}
    else if (sec!=50 ||sec!=43 || Water_level == 0)
...
//  Serial.println(Interval);
  ...
  if(sec<=time_to_turn_off_Grow_lights|| sec>=time_to_start_Grow_lights)
  ...
  if ( (pH_starting==true&& ((sec==41||sec==42|| sec==43|| sec==44||sec==45 ||sec==46))))
  {
...
else if ((sec!=41||sec!=46))
{
  ...
  if(tds_starting_time==true && day_n%7==0 &&(sec==18||sec==19|| sec==20))
  {
...
unsigned long   curMillis_2 = millis();
  if ((curMillis_2 - prevMillis_2) >=400 &&(sec==0|| sec==1 ||sec==3 ||sec==4||sec==5||sec==6||sec==7) )
  {
  ...
  if (( curMillis_2- prevMillis_2) >= 400 && (sec==8||sec==9||sec==10||sec==11||sec==12)  )
  {
    //issues fixed
    prevMillis_2 = curMillis_2;
    lcd.setCursor(0, 1);
  ...
  if (( curMillis_2- prevMillis_2) >= 400 && ( sec==13 ||sec==14||sec==15 ||sec==16||sec==17||sec==18 ||sec==19||sec==20))
  {
    prevMillis_2 = curMillis_2;
   lcd.setCursor(0, 1);
    lcd.print(F("Temp:"));
  ...
  if (( curMillis_2- prevMillis_2) >= 400 &&( sec==21|| sec==22 ||sec==23 ||sec==24||sec==25||sec==26||sec==27))
  {
  ...
  if ((curMillis_2 - prevMillis_2) >= 400 && (sec==28||sec==29||sec==30||sec==31||sec==32||sec==33||sec==34 ))
  {
  //issues fixed 
   prevMillis_2 = curMillis_2;
    lcd.setCursor(0, 1);
  ...
  if (( curMillis_2- prevMillis_2) >= 400 && (sec==35||sec==36||sec==37||sec==38||sec==39||sec==40||sec==41||sec==42||sec==43))
  {
    prevMillis_2 = curMillis_2;
    lcd.setCursor(0, 1);
    lcd.print(F("EC:")); 
  ...
  if (( curMillis_2 - prevMillis_2) >= 400  && (sec==44||sec==45||sec==46||sec==47||sec==48||sec==49||sec==50||sec==51))
  {
    prevMillis_2 = curMillis_2;
    lcd.setCursor(0, 1);
    lcd.print(F("Tempe 2:")); 
